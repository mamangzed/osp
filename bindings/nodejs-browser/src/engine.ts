/**
 * OSP Sync Engine - Vector Clock, LWW merge, offline-first storage (Browser SDK)
 * Note: Browser version uses in-memory Map only (no SQLite).
 */
...
export class SyncEngine {
  ...
  listRecords(coll: CollectionId, predicate?: Predicate): RecordMsg[] {
    ...
    for (const [id, rec] of collection.records) {
      ...
      if (recMsg && (!predicate || this.matchesPredicate(recMsg, predicate))) {
        records.push(recMsg);
      }
    }
    return records;
  ...
  private matchesPredicate(rec: RecordMsg, pred: Predicate): boolean {
    ...
    switch (pred.kind) {
      ...
      case PredicateKind.And:
        return pred.children!.every((child) => this.matchesPredicate(rec, child));
      case PredicateKind.Or:
        return pred.children!.some((child) => this.matchesPredicate(rec, child));
      case PredicateKind.Not:
        return !this.matchesPredicate(rec, pred.child!);
      default:
        return true;
    }
  }

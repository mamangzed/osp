/**
 * OSP Wire Protocol - 22-byte fixed header + JSON payload (Browser SDK)
 */
...
export class FrameHeader {
  ...
  encode(): Uint8Array {
    ...
    buf.set(magicBytes, 0);

    const view = new DataView(buf.buffer);
    view.setUint16(4, this.version, false); // Big-endian
    view.setUint16(6, this.opcode, false);
    view.setUint16(8, this.flags, false);
    view.setUint32(10, this.length, false);
    // For req_id (bigint), we need to write high and low 32 bits
    const high = Number(this.req_id >> 32n);
    const low = Number(this.req_id & 0xFFFFFFFFn);
    view.setUint32(14, high, false);
    view.setUint32(18, low, false);

    return buf;
  }

  static decode(buf: Uint8Array): FrameHeader {
    ...
    const opcodeRaw = view.getUint16(6, false);
    const flags = view.getUint16(8, false);
    const length = view.getUint32(10, false);
    const high = view.getUint32(14, false);
    const low = view.getUint32(18, false);
    const req_id = (BigInt(high) << 32n) | BigInt(low);
...
export class Frame {
  ...
  static create(opcode: OpCode, req_id: bigint, payload: Uint8Array, flags: number = 0): Frame {
    return new Frame(
...
export class Envelope {
  ...
  static decode(opcode: OpCode, buf: Uint8Array): Envelope {
